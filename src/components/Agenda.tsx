      ...
      <div className="max-w-4xl mx-auto space-y-6">
        {agenda.map((item, index) => (
          <div
            ...
            className={`relative transform-gpu rounded-xl p-6 sm:p-8 border border-opacity-50 border-l-4 bg-[#1a1a2e]/50 backdrop-blur-sm shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-2xl
              ${
                item.color === "purple"
                  ? "border-[#e6ceff] border-l-[#d8b4fe] text-[#e9d5ff]"
                  : "border-[#bde5f3] border-l-[#a5e8ff] text-[#bae6fd]"
              }`}
            role="listitem"
            tabIndex={0}
            aria-label={`Agenda item: ${item.title} from ${item.time}`}
          >
            <div className="flex items-center gap-3 text-sm sm:text-base text-gray-300 mb-2">
