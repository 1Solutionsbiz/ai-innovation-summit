...
const RoomThatMatters = () => {
  ...
  const delayFor = (index: number, total: number) => {
    ...
    return isVisible ? `${order * 120}ms` : undefined;
  };
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden border-t-[5px] border-[#07163d] bg-[#f2f4ff] px-4 py-10 sm:px-8 sm:py-12 md:py-16 lg:px-12">
      ...
      <div className="mx-auto max-w-[1700px] text-center">
        <h2
          className={`partners-reveal-up text-[#06245d]
                  font-black
                  text-[34px]
                  sm:text-[48px]
                  md:text-[64px]
          ...
          style={{ animationDelay: delayFor(0, 2) }}
        >
          The Room That Matters
        </h2>
        <p
          className={`partners-reveal-up mt-4 text-base font-normal text-[#171717] sm:mt-6 sm:text-xl md:text-1xl ${isVisible ? "is-visible" : ""}`}
          style={{ animationDelay: delayFor(1, 2) }}
        >
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 sm:mt-16 sm:grid-cols-2 sm:gap-6 lg:grid-cols-5 lg:gap-12 xl:gap-14">
          {CATEGORIES.map((category, idx) => {
            return (
              <div
                key={idx}
                className={`partners-reveal-up flex min-h-[240px] flex-col items-center justify-start rounded-[18px] border-2 border-[#d8d9ee] bg-white/75 px-4 py-6 shadow-[0_2px_8px_rgba(36,51,110,0.03)] transition-shadow hover:shadow-[0_8px_24px_rgba(36,51,110,0.12)] sm:min-h-[300px] sm:rounded-[24px] sm:px-5 sm:py-7 ${isVisible ? "is-visible" : ""}`}
                style={{ animationDelay: delayFor(idx, CATEGORIES.length) }}
              >
                <div
        ...
        {/* Industries strip */}
        <div
          className={`partners-reveal-up mt-8 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 sm:mt-10 sm:gap-x-4 sm:gap-y-3 lg:flex-nowrap lg:gap-x-5 ${isVisible ? "is-visible" : ""}`}
          style={{ animationDelay: delayFor(0, 1) }}
        >
